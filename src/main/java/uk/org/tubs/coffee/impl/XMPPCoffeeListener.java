...
//			conn.login("coffee", "c0ff33");
	...
	private Presence getPresence(CoffeeStatus s) {
		...
		if(!s.isBrewing() && s.isHotplateOn()){
			...
			String brewedAgo = HumanTime.approximately(System.currentTimeMillis() - lastFinishedBrewing);
			if("".equals(brewedAgo)){
				brewedAgo = "a couple of secs";
			}
			brewedAgo = "Brewed "+ brewedAgo + " ago.";
			p = new Presence(Presence.Type.available, brewedAgo ,1,Mode.available);
		}
